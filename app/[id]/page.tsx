...
export default function GameDetailPage() {
    ...
    const renderStatusIcon = (status: string) => {
        switch (status) {
            case "completed":
                return <CheckCircle2 className={`w-4 h-4 sm:w-5 sm:h-5 ${theme.colorCompleted}`}/>;
            case "in-progress":
                return <Clock className={`w-4 h-4 sm:w-5 sm:h-5 ${theme.colorInProgress}`}/>;
            case "planned":
                return <Circle className={`w-4 h-4 sm:w-5 sm:h-5 ${theme.textMuted}`}/>;
            case "to-decide":
                return <CircleQuestionMark className={`w-4 h-4 sm:w-5 sm:h-5 ${theme.colorToDecide}`}/>;
            default:
                return <Circle className={`w-4 h-4 sm:w-5 sm:h-5 ${theme.accentText}`}/>;
        }
    };
        ...
        <div className={`${theme.bgColor} min-h-screen`}>
            ...
            >
                ...
                <div className="relative z-10 w-full mx-auto px-6 sm:px-8 lg:px-16 text-center">
                    ...
                    <p
                        className={`text-lg sm:text-2xl lg:text-3xl ${theme.textMuted} mb-6 sm:mb-8 tracking-wide`}
                        style={{ fontFamily: theme.fontBody, fontWeight: 200 }}
                    >
                        {gameT.subtitle}
                    </p>
                    <div className={`flex items-center justify-center gap-4 sm:gap-6 mb-8 sm:mb-12`}>
                        <div className={`w-12 sm:w-24 h-[2px] ${theme.accentBg}`}></div>
                    ...
                    <div className="flex flex-wrap gap-3 sm:gap-4 justify-center">
                        {game.links.steam && (
                            <ThemedLink href={game.links.steam} theme={theme}>
                                <ExternalLink className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${theme.accentText}`}/>
                                <span className={`tracking-wider text-xs sm:text-sm ${theme.fontBodyColor}`} style={{ fontFamily: theme.fontBody }}>Steam</span>
                            </ThemedLink>
                        )}
                        {game.links.demo && (
                            <ThemedLink href={game.links.demo} theme={theme}>
                                <Play className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${theme.accentText}`}/>
                                <span className={`tracking-wider text-xs sm:text-sm ${theme.fontBodyColor}`} style={{ fontFamily: theme.fontBody }}>Demo</span>

                            </ThemedLink>
                        )}
                        {game.links.github && (
                            <ThemedLink href={game.links.github} theme={theme}>
                                <Github className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${theme.accentText}`}/>
                                <span className={`tracking-wider text-xs sm:text-sm ${theme.fontBodyColor}`} style={{ fontFamily: theme.fontBody }}>Github</span>
                            </ThemedLink>
                        )}
                        {game.links.pressKit && (
                            <ThemedLink href={game.links.pressKit} theme={theme}>
                                <FileText className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${theme.accentText}`} style={{ fontFamily: theme.fontBody }}/>
                                <span className={`tracking-wider text-xs sm:text-sm ${theme.fontBodyColor}`} style={{ fontFamily: theme.fontBody }}>Press Kit</span>
                            </ThemedLink>
                        )}
                    </div>
                </div>
                    ...
                    <div className={`border-2 ${theme.panelBorderOpacity}`}>
                        ...
                        >
                            <CardGrid
                                ...
                                cornerClass={`border-t-2 border-r-2 ${theme.cardBorderOpacity} ${theme.panelBorderHover} absolute top-3 right-3 sm:top-4 sm:right-4 w-8 h-8 sm:w-12 sm:h-12`}
                                borderClass={`border-2 ${theme.cardBorderOpacity} ${theme.panelBorderHover}`}
                        ...
                        >
                            <CardGrid
                                items={gameT.market}
                                cornerClass={`border-b-2 border-l-2 ${theme.cardBorderOpacity} ${theme.panelBorderHover} absolute bottom-3 left-3 sm:bottom-4 sm:left-4 w-8 h-8 sm:w-12 sm:h-12`}
                                borderClass={`border-2 ${theme.cardBorderOpacity} ${theme.panelBorderHover}`}
                                textSubtle={theme.textSubtle}
                                textMuted={theme.textMuted}
                                fontBody={theme.fontBody}
                                ...
                                {gameT.technic.map((technic: {title: string; description: string}, i: number) => (
                                    <div key={i} className={`border-2 ${theme.cardBorderOpacity} p-4 sm:p-6 ${theme.panelBorderHover} transition-all duration-300 relative group`}>
                                        <div className="flex items-start gap-3 sm:gap-4">
                                            ...
                                            <div>
                                                <h4 className={`text-sm sm:text-lg mb-1 sm:mb-2 tracking-wide ${theme.textSubtle}`} style={{ fontFamily: theme.fontBody, fontWeight: "bold" }}>
                                                    {technic.title}
                                                </h4>
                                                <p className={`text-xs sm:text-base ${theme.textMuted}`} style={{ fontFamily: theme.fontBody }}>
                                                    {technic.description}
                                                </p>
                                            </div>
                                        </div>
            ...
            >
                <div className="container mx-auto px-4 sm:px-8 lg:px-16">
                    <SectionHeading
                        title={t.gamePage.roadmap.title}
                        subtitle={t.gamePage.roadmap.subtitle}
                        fontTitle={theme.fontTitle}
                        fontBody={theme.fontBody}
                        accentBg={theme.accentBg}
                        fontTitleColor={theme.fontTitleColor}
                    ...
                    <div className={`border-2 ${theme.panelBorderOpacity}`}>
                        <PanelBlock
                            heading={t.gamePage.roadmap.vision.title}
                            align="left"
                            fontBody={theme.fontBody}
                            accentBg={theme.accentBg}
                            fontBodyColor={theme.fontBodyColor}
                        >
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8">
                                <div className={`border-2 ${theme.cardBorderOpacity} p-5 sm:p-8 ${theme.panelBorderHover} transition-all duration-300 relative group`}>
                                    ...
                                    <p className={`text-xs sm:text-base ${theme.textMuted} leading-relaxed`} style={{ fontFamily: theme.fontBody }}>
                                        {gameT.vision}
                                    </p>
                                </div>
                                <div className={`border-2 ${theme.cardBorderOpacity} p-5 sm:p-8 ${theme.panelBorderHover} transition-all duration-300 relative group`}>
                                ...
                                {gameT.roadmap.map((phase: {phase: string; status: string; items: string[]}, i: number) => (
                                    <div key={i} className={`border-2 ${theme.panelBorderOpacity} p-5 sm:p-8`}>
                                        <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6 flex-wrap">
                                            ...
                                            <h3 className={`text-base sm:text-2xl tracking-wider flex-1 ${theme.fontBodyColor}`} style={{ fontFamily: theme.fontBody }}>
                                                {phase.phase}
                                            </h3>
                ...
                <div className="container mx-auto px-4 sm:px-8 lg:px-16">
                    <div className="max-w-5xl mx-auto">
                        <div className="text-center mb-6 sm:mb-7">
                            <h2 className={`text-3xl sm:text-5xl lg:text-6xl mb-4 sm:mb-6 tracking-wider ${theme.fontTitleColor}`} style={{ fontFamily: theme.fontTitle }}>
                                {t.gamePage.investment.title}
                            </h2>
                            <div className="flex items-center justify-center gap-3 sm:gap-4">
                                <div className={`w-12 sm:w-20 h-[2px] ${theme.accentBg}`}/>
                                <p className={`text-xs sm:text-sm tracking-[0.25em] sm:tracking-[0.3em] uppercase ${theme.fontBodyColor}`} style={{ fontFamily: theme.fontBody }}>
                        ...
                        <div className="space-y-4 sm:space-y-8 mb-8 sm:mb-12">
                            {[
                                ...
                                <div key={i} className={`border-2 ${theme.panelBorderOpacity} p-5 sm:p-8`}>
                                    <h3 className={`text-lg sm:text-2xl mb-3 sm:mb-4 tracking-wider ${theme.textSubtle}`} style={{ fontFamily: theme.fontBody }}>
                                        {label}
                                    </h3>
                                    <p className={`text-xs sm:text-base ${theme.textMuted} tracking-wide leading-relaxed`} style={{ fontFamily: theme.fontBody }}>
                                        {content}
                                    </p>
                                </div>
                            ))}
                        ...
                        <div className={`border-2 ${theme.investmentHighlightBorder} p-5 sm:p-8 ${theme.investmentHighlightBg}`}>
                            <h3 className={`text-lg sm:text-2xl mb-4 sm:mb-6 tracking-wider ${theme.textSubtle}`} style={{ fontFamily: theme.fontBody }}>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                                {gameT.investment.keys.map((key: string, i: number) => (
                                    <div key={i} className="flex items-start gap-2 sm:gap-3">
                                        ...
                                        <span className={`text-xs sm:text-base ${theme.textSubtle} tracking-wide`} style={{ fontFamily: theme.fontBody }}>
                                            {key}
                                        </span>
                                    </div>
                                ))}
            ...
            <section id="contact" className={`${theme.bgColor} py-16 sm:py-24 border-t-2 ${theme.panelBorderOpacity}`}>
                <div className="container mx-auto px-4 sm:px-8 lg:px-16">
                    <div className="max-w-4xl mx-auto text-center">
                        <h2 className={`text-3xl sm:text-4xl lg:text-5xl mb-4 sm:mb-6 tracking-wider ${theme.fontTitleColor}`} style={{ fontFamily: theme.fontTitle }}>
                            {t.gamePage.contact.title}
                        </h2>
                        <p className={`text-sm sm:text-xl ${theme.textMuted} mb-8 sm:mb-12 tracking-wide`} style={{ fontFamily: theme.fontBody }}>
