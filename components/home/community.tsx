...
import { SOCIALS, getSocials, type SocialLink } from "@/libs/database/socialsData";

type CommunitySocialId = "twitter" | "youtube" | "instagram" | "tiktok";

const socials = getSocials(["twitter", "youtube", "instagram", "tiktok"]) as (SocialLink & {
  id: CommunitySocialId;
})[];
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            <div className="w-20 h-20 border-2 border-white flex items-center justify-center group-hover:bg-white/5 transition-all duration-300">
              <SOCIALS.discord.icon className="w-9 h-9 text-white/70" />
            </div>
            <div className="flex-1 text-center md:text-left">
              <p
                className="text-xs tracking-[0.3em] uppercase text-white/40 mb-2"
                style={{ fontFamily: "Cinzel" }}
              >
                {tt.discord.title}
              </p>
          ...
          <div className="container mx-auto px-8 lg:px-16 mt-16">
            <div className="max-w-6xl mx-auto">
              <a
                href={SOCIALS.coffee.url}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-between gap-6 border border-white/20 hover:border-white/50 px-8 py-5 transition-all duration-300 group"
              >
                <div className="flex items-center gap-4">
                  <Coffee className="w-5 h-5 text-white/40 group-hover:text-white/70 transition-colors shrink-0" />
                  <div>
                    <p
                      className="text-sm tracking-wider text-white/70 group-hover:text-white transition-colors"
                      style={{ fontFamily: "Cinzel", fontWeight: 400 }}
                    >
                      {tt.coffee.title}
                    </p>
