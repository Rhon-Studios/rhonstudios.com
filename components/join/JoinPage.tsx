...
export default function JoinPage() {
  ...
  const conditions: ConditionItem[] = tt.conditions.items;
  ...
  const rolesInScope = opportunitiesData.filter(
    (o) => projectFilter === "all" || o.projectId === projectFilter
  );
  const areas: Array<"all" | AreaFilter["area"]> = [
    "all",
    ...Array.from(new Set(opportunitiesData.map((opp) => opp.area))),
  ];
  ...
  return (
    <div className="bg-black min-h-screen">
      ...
      >
        <div className="container mx-auto px-6 sm:px-8 lg:px-16">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {conditions.map((c: ConditionItem, i: number) => {
                                ...
                                <ul className="space-y-2">
                                  {oppT.requirements.map((r: string, i: number) => (
                                    <li
                                      ...
                                      style={{ fontFamily: "Cinzel", fontWeight: 200 }}
                                    >
                                      <span className="mt-1 text-white shrink-0">✦</span>
                                      {r}
                                    </li>
                                  ))}
                                </ul>
      ...
      <AnimatePresence>
        {modal && <ApplicationModal initial={modal} onClose={() => setModal(null)} />}
      </AnimatePresence>
    </div>
  );
}
