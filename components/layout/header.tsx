...
const HeaderBackground = ({ isScrolled }: { isScrolled: boolean }) => (
  <div
    aria-hidden
...
export function Header() {
  ...
  const headerLogo = isIdPage && game?.logo ? game.logo : "/logos/RhonStudiosCircleLogo.png";
  const headerLogoLarge = isIdPage && game?.logo ? game.logo : "/logos/IconHeader.png";
  ...
  const devblogItems = [{ id: "devblog_idex", label: t.devblog_menu.devblogs }];
    ...
    <div className="fixed inset-0 z-200 bg-black/98 flex flex-col pt-24 pb-10 px-6 overflow-y-auto">
      ...
      <div className="flex flex-col gap-4 mt-4">
        {onHome && (
          ...
          >
            <h3
              className="text-lg tracking-wider uppercase text-white/80"
      ...
      {titleLeft && (
        <div className="hidden xl:flex items-center gap-4 absolute left-[32%] -translate-x-full">
          <h1
            ...
            style={{ fontFamily: titleFont, textShadow: "0 4px 20px rgba(255,255,255,0.15)" }}
          >
            {titleLeft}
          </h1>
        </div>
  ...
  if (isJoinPage) {
    return (
      <header className="fixed top-0 left-0 right-0 z-50">
        ...
        <nav
          className={`relative container mx-auto px-4 sm:px-8 lg:px-16 transition-all duration-500 ${isScrolled ? "py-2 lg:py-4" : "py-4 lg:py-8"}`}
        >
          <MobileBar onBack={() => router.push("/")} />
        ...
        <AnimatePresence>
          {isScrolled && isMenuOpen && (
            <div
              ref={menuRef}
              className="hidden lg:block relative overflow-hidden bg-black/98 backdrop-blur-sm border-t-2 border-white/20"
            >
  ...
  if (isIdPage) {
    return (
      <header className="fixed top-0 left-0 right-0 z-50">
        <HeaderBackground isScrolled={isScrolled} />
        <nav
          className={`relative container mx-auto transition-all duration-500 ${isScrolled ? "py-2 lg:py-6" : "py-4 lg:py-10"}`}
        >
          <MobileBar logo={headerLogo} alt={game?.title ?? "Rhon Studios"} />
  ...
  if (isCollabPage) {
    return (
      <header className="fixed top-0 left-0 right-0 z-50">
        <HeaderBackground isScrolled={isScrolled} />
        <nav className="relative container mx-auto py-4 lg:py-6 px-4 sm:px-8 lg:px-16 flex items-center justify-between">
          <button
